...
def locateScrew(data, plan, positions):
    ...
    print(f"x_aligned: {abs(positions[-1][0]-plan['x'])}, y_aligned: {abs(positions[-1][1]-plan['y'])}")
...
def requirement_process(data, previous_data, positions):
    ...
    if located and xy_stable and aligned:
        last_min_angelZ = min(data['angle']['z'] for data in previous_data[-8:-5])
        if abs(data['angle']['z'] - previous_data[-1]['angle']['z']) > 15:
