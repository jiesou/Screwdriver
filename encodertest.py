...
def read_encoder_value(ser, slave_id=0x01):
    ...
    if received_crc != calculated_crc:
        print(f"CRC 校验失败: 接收={received_crc:04X}, 计算={calculated_crc:04X}")
        return None
    
    # 验证从站地址和功能码
    if response[0] != slave_id or response[1] != 0x03:
        print(f"响应格式错误: {response.hex()}")
        return None
    
    # 解析编码器值 (4字节，大端序)
    byte_count = response[2]
    if byte_count != 4:
        print(f"数据字节数错误: {byte_count}")
        return None
    ...
    while True:  # 外层无限循环用于重连
        try:
            with serial.Serial(port, baudrate, timeout=timeout) as ser:
                print(f"拉线编码器串口已连接: {port}, 波特率: {baudrate}, 从站地址: {slave_id}")
